...
export function HomeView({
  ...
  return (
    ...
    const cardTx = (transactions || []).filter((t: any) => {
      const isCardMatch = t.card_id && card.id && String(t.card_id).toLowerCase() === String(card.id).toLowerCase();
    ...
    let nextDeadlineFmt = null;
    let customUrgency = "normal";

    if (cardTx.length > 0 && info.hasDebt) {
      // Сортируем транзакции по возрастанию дат
      ...
      const nextDeadlineStr = sortedTx[0].credit_due_date; 
      ...
      if (nextDeadlineStr) {
        ...
        displayDaysLeft = Math.ceil(diffTime / (1000 * 60 * 60 * 24));
        if (displayDaysLeft < 0) displayDaysLeft = 0;

        nextDeadlineFmt = nextDeadlineStr.split("-").reverse().join("."); // Формат "28.09.2026"
        customUrgency = displayDaysLeft <= 7 ? "danger" : displayDaysLeft <= 14 ? "warn" : "normal";
      }
    } else if (info.hasDebt) {
      displayDaysLeft = 30; // Страховочный дефолт
    }
      ...
      <div key={card.id} className="rounded-xl bg-slate-800/50 border border-slate-700/50 p-3">
        <div className="flex items-center justify-between mb-2">
          ...
          <span className={`text-xs font-semibold ${
            customUrgency === "danger" ? "text-rose-400" : customUrgency === "warn" ? "text-amber-400" : "text-emerald-400"
          }`}>
            {!info.hasDebt ? (
              "Долгов нет"
            ) : nextDeadlineFmt ? (
              <>До {nextDeadlineFmt} ({displayDaysLeft} дн.)</>
            ) : (
              <>{displayDaysLeft} дн. без %</>
            )}
          </span>
        </div>
        <div className="flex items-center justify-between text-xs">
          <span className="text-slate-500">Долг: <span className={info.hasDebt ? "text-rose-400 font-medium" : "text-slate-400"}>{info.debt.toLocaleString()} ₽</span></span>
          <span className="text-slate-500">Доступно: <span className="text-white font-medium">{(card.limit - info.debt).toLocaleString()} ₽</span></span>
        </div>
      </div>
    );
